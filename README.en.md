...
- 2026-06-11: Added a reading-mode table-comment regression test that simulates the 7 table threads from `local/reply.local.md`, including Obsidian splitting one raw thread across `<code>`, `<strong>`, `<br>`, and detached `div.table-cell-wrapper` DOM fragments. Verified that all 7 comments render and the raw `{>>...<<}` markup is fully hidden. Validation: `npm test` 84/84 PASS, `npx tsc --noEmit` PASS, `npm run build` PASS.
